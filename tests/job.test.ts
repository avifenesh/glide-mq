...
describe('Job', () => {
  ...
  describe('updateData', () => {
    it('should update the data field in the hash and locally', async () => {
      const job = new Job<any>(mockClient as any, keys, '1', 'job', { old: true }, {});

      const newData = { updated: true, count: 5 };
      await job.updateData(newData);

      expect(mockClient.hset).toHaveBeenCalledWith('glide:{test-queue}:job:1', {
  ...
  describe('getChildrenValues', () => {
    ...
    it('should fetch return values from child jobs', async () => {
      mockClient.smembers.mockResolvedValue(
        new Set(['glide:{test-queue}:10', 'glide:{test-queue}:11']),
      );
