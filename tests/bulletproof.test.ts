...
describeEachMode(
  'BullMQ #3767: getJobs(active) never returns undefined/null entries',
  (CONNECTION) => {
...
describeEachMode(
  'BullMQ #3133: no duplicate scheduler job execution within same interval',
  (CONNECTION) => {
  ...
  it('parent transitions from waiting-children to completed after all children finish', async () => {
    ...
    const done = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(
        () => reject(new Error('timeout - parent stuck in waiting-children')),
        15000,
      );
      const worker = new Worker(
        Q,
...
describeEachMode(
  'BullMQ swallowed errors: worker emits error events on job failure',
  (CONNECTION) => {
...
describeEachMode(
  'BullMQ #2166: FlowProducer add+close does not leak event listeners',
  (CONNECTION) => {
...
describeEachMode(
  'Celery #4354: worker shutdown during processing does not lose jobs',
  (CONNECTION) => {
...
describeEachMode(
  'Celery visibility timeout: heartbeat prevents false reclaim of long jobs',
  (CONNECTION) => {
...
describeEachMode(
  'Celery #7801: results do not accumulate when removeOnComplete is set',
  (CONNECTION) => {
...
describeEachMode('Celery #8030: worker resumes processing after reconnect cycle', (CONNECTION) => {
  ...
  it('new worker processes jobs added after previous worker is gone', async () => {
    ...
    const secondWorkerDone = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(
        () => reject(new Error('second worker did not process all jobs')),
        15000,
      );
...
describeEachMode(
  'Sidekiq BRPOP loss: job not lost when worker crashes mid-processing',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq dedup lock orphan: dedup lock not orphaned after completion',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq retry loss: job exists in a known state throughout retry cycle',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq counter race: job IDs are monotonically increasing under concurrent adds',
  (CONNECTION) => {
...
describeEachMode(
  'Bee-Queue #106: repeated stalling moves job to failed, not infinite loop',
  (CONNECTION) => {
...
describeEachMode(
  'Bee-Queue #584: close(false) with active job returns within timeout',
  (CONNECTION) => {
...
describeEachMode(
  'Bee-Queue #885: processing many jobs does not leak event listeners',
  (CONNECTION) => {
    ...
    it('QueueEvents + 50 jobs produces no MaxListenersExceededWarning', async () => {
      ...
      try {
        const { QueueEvents } =
          require('../dist/queue-events') as typeof import('../src/queue-events');
...
describeEachMode(
  'Sidekiq BRPOP immunity: job in PEL survives worker crash and is reclaimed',
  (CONNECTION) => {
...
describeEachMode('Sidekiq dedup: unique lock freed after job completion', (CONNECTION) => {
  ...
  it('dedup id=unique-1 reusable after first job completes', async () => {
    ...
    const firstDone = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('timeout')), 15000);
      const worker = new Worker(Q, async () => 'done-1', {
        connection: CONNECTION,
        concurrency: 1,
        blockTimeout: 500,
      });
    ...
    const secondDone = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('timeout')), 15000);
      const worker = new Worker(Q, async () => 'done-2', {
        connection: CONNECTION,
        concurrency: 1,
        blockTimeout: 500,
      });
...
describeEachMode(
  'Sidekiq batch: parent completes exactly once when all children finish',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq retry: job never missing from all state structures during retry',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq scheduler drift: interval jobs fire without drift accumulation',
  (CONNECTION) => {
    ...
    it('scheduler every=1000ms fires 3 times in ~4s with gaps within 500ms tolerance', async () => {
      ...
      const totalSpan =
        processedTimestamps[processedTimestamps.length - 1] - processedTimestamps[0];
...
describeEachMode(
  'Sidekiq pool exhaustion: rapid create/close does not leak connections',
  (CONNECTION) => {
...
describeEachMode(
  'Sidekiq large payload: 500KB job data roundtrips without corruption',
  (CONNECTION) => {
