  ...
  describe('Retention edge: removeOnComplete with age and count', () => {
    ...
    it('removeOnComplete: {age: 1, count: 100} - old jobs removed by age', async () => {
      ...
      const done = new Promise<void>((resolve, reject) => {
        ...
        const worker = new Worker(Q, async () => 'ok', { connection: CONNECTION, concurrency: 1, blockTimeout: 1000 });
        worker.on('completed', () => {
          processed++;
          if (processed >= TOTAL) {
            clearTimeout(timeout);
      ...
      const done2 = new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('timeout')), 10000);
        const worker2 = new Worker(Q, async () => 'ok', { connection: CONNECTION, concurrency: 1, blockTimeout: 1000 });
