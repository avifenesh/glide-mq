 ...
 * Tests the lifo option for queue.add() and verifies reverse-chronological processing.
 */
...
describeEachMode('LIFO: Priority interactions', (CONNECTION) => {
  ...
  it('delayed LIFO jobs are promoted to LIFO list', async () => {
    ...
    // Delayed job promoted to LIFO list, should process before immediate job
...
describeEachMode('LIFO: Mixed with FIFO', (CONNECTION) => {
  ...
  it('LIFO jobs process before FIFO jobs', async () => {
    ...
    // LIFO jobs in reverse order (lifo2, lifo1), then FIFO (fifo1, fifo2)
