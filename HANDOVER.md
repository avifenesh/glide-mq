...
- **Ordered groups**: pre-activation removal, revoke, and expiry close ordering holes; rejected token-bucket enqueue operations do not consume IDs or sequences; rate-limit requeues preserve their slot; priority-list fast fetches apply token-bucket and fixed-window gates. `LIBRARY_VERSION` 101.
